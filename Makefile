...
ifeq ($(filter $(1),simposix realposix simwroom simlitewing),)
...
PACKAGE_FW_EXCLUDE  := fw_simposix fw_realposix fw_simwroom fw_simlitewing $(if $(PACKAGE_FW_INCLUDE_DISCOVERYF4BARE),,fw_discoveryf4bare)
PACKAGE_FW_TARGETS  := $(filter-out $(PACKAGE_FW_EXCLUDE), $(FW_TARGETS))
PACKAGE_ELF_TARGETS := $(filter     fw_simposix fw_realposix fw_simwroom fw_simlitewing, $(FW_TARGETS))
