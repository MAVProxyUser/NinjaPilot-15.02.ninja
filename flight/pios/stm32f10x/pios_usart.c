...
#if defined(PIOS_INCLUDE_FREERTOS)
    /* vPortYield() (no-arg) doesn't exist in the current FreeRTOS-Kernel
     * ARM_CM3 port - portYIELD_FROM_ISR(x) is the standard cross-port
     * macro now, and already checks x itself. */
